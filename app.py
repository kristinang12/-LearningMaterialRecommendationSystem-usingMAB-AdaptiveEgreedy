...
import psycopg2, random, os, uuid, math, numpy as np
from flask_sqlalchemy import SQLAlchemy

app = Flask(__name__)

app.secret_key = 'Adaptive-egreedyKey'
...
def regretCalculation(optimal, arm_id_r, meanreward_r, session_key, meanreward_s=None, arm_id_s=None):
    if arm_id_s is not None:
        ...
        cur.execute("INSERT INTO regretcalculationaeg (arm_id_s,optimalarm, selectedarm_s, regret_s, arm_id_r, selectedarm_r, regret_r, total_regret, session_key) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", (arm_id_s, optimal, meanreward_s, regret_s, arm_id_r, meanreward_r, regret_r, total_regret, session_key))
    else:
        regret_r = optimal - meanreward_r
        total_regret = regret_r
        cur.execute("INSERT INTO regretcalculationaeg (arm_id_r,optimalarm, selectedarm_r, regret_r, total_regret, session_key) VALUES (%s, %s, %s, %s, %s, %s)", (arm_id_r,optimal,  meanreward_r, regret_r, total_regret, session_key))
    
    conn.commit()

def rewardCalculation(arm_id_r, session_key, arm_id_s=None):
    cur.execute("SELECT alpha, beta, average_reward FROM armsrewardaeg WHERE arm_id = %s", (arm_id_r,))
    row_r = cur.fetchone()
    ...
    if row_r is not None:
        ...
        if arm_id_s is not None:
            cur.execute("SELECT alpha, beta, average_reward FROM armsrewardaeg WHERE arm_id = %s", (arm_id_s,))
            row_s = cur.fetchone()
            ...
            if row_s is not None:
                ...
                cur.execute("INSERT INTO rewardcalculationaeg (arm_id_s, alpha_s, beta_s, meanreward_s, arm_id_r, alpha_r, beta_r, meanreward_r, total_meanreward, session_key) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (arm_id_s, alpha_s, beta_s, meanreward_s, arm_id_r, alpha_r, beta_r, meanreward_r, total_meanreward, session_key))
                
                cur.execute("SELECT average_reward FROM armsrewardaeg ORDER BY average_reward DESC LIMIT 1")
            ...
            cur.execute("INSERT INTO rewardcalculationaeg (arm_id_r, alpha_r, beta_r, meanreward_r, total_meanreward, session_key) VALUES (%s, %s, %s, %s, %s, %s)", (arm_id_r, alpha_r, beta_r, meanreward_r, total_meanreward, session_key))
...
def observereward(arm_id, session_key):
    cur.execute("SELECT id, arm_id_s, alpha_s, beta_s, meanreward_s, arm_id_r, alpha_r, beta_r, meanreward_r FROM rewardcalculationaeg WHERE session_key <= %s ORDER BY id DESC", (session_key,))
    rows = cur.fetchall()
    ...
    if rows:
        ...
        if arm_id == arm_id_search:
            ...
            reward_s = 1
            cur.execute("UPDATE rewardcalculationaeg SET reward_s = %s, alpha_s = %s, beta_s = %s, meanreward_s = %s, total_meanreward =%s WHERE session_key = %s", (reward_s, alpha_s, beta_s, meanreward_s, total_meanreward, session_key))
            
            cur.execute("SELECT meanreward_r, meanreward_s FROM rewardcalculationaeg WHERE session_key <= %s ORDER BY id DESC", (session_key,))
            whatOptimal = cur.fetchone()

            if whatOptimal: 
                meanreward_r_main, meanreward_s_main = whatOptimal
                if meanreward_s_main > meanreward_r_main:
                    cur.execute("SELECT optimalarm, regret_r FROM regretcalculationaeg WHERE session_key = %s", (session_key,))
                    ...
                    if regretrows:
                        ...
                        if  meanreward_s_main > optimal_arm:
                            ...
                            cur.execute("UPDATE regretcalculationaeg SET optimalarm=%s, selectedarm_s=%s, regret_s = %s, total_regret=%s WHERE session_key=%s", (optimal, meanreward_s, regret_s,  total_regret, session_key))
                        else:
                            optimal = optimal_arm
                            regret_s = optimal - meanreward_s
                            total_regret = (regret_r + regret_s)/2
                    ...
                    if regretrows:
                        ...
                        if meanreward_r_main > optimal_arm:
                            ...
                            regret_s = optimal - meanreward_s
                            total_regret = (regret_r + regret_s)/2
                            cur.execute("UPDATE regretcalculationaeg SET optimalarm=%s, selectedarm_s=%s, regret_s = %s, selectedarm_r=%s, regret_r=%s, total_regret=%s WHERE session_key=%s", (optimal, meanreward_s, regret_s, meanreward_r, regret_r, total_regret, session_key))
                        else:
                            optimal = meanreward_r_main
                            regret_s = optimal - meanreward_s
                            total_regret = (regret_r + regret_s)/2
            ...
            total_meanreward = meanreward_r
            reward_r = 1
            cur.execute("UPDATE rewardcalculationaeg SET reward_r = %s, alpha_r = %s, beta_r = %s, meanreward_r = %s, total_meanreward =%s  WHERE session_key = %s", (reward_r, alpha_r, beta_r, meanreward_r, total_meanreward, session_key))
                ...
                if meanreward_r_main > meanreward_s_main:
                    ...
                    if regretrows:
                        ...
                        if meanreward_r_main > optimal_arm:
                            ...
                            regret_r = optimal - meanreward_r
                            total_regret = regret_r
                            cur.execute("UPDATE regretcalculationaeg SET optimalarm=%s, selectedarm_r=%s, regret_r=%s, total_regret=%s WHERE session_key=%s", (optimal, meanreward_r, regret_r, total_regret, session_key))
                        else:
                            optimal = optimal_arm
                            regret_r = optimal - meanreward_r
                            total_regret = regret_r 
...
def updateReward(arm_id):
    cur.execute("SELECT alpha, beta, average_reward FROM armsrewardaeg WHERE arm_id = %s", (arm_id,))
    row = cur.fetchone()
    ...
    if row is not None:
        ...
        cur.execute("UPDATE armsrewardaeg SET alpha = %s, beta = %s, average_reward = %s WHERE arm_id = %s", (alpha, beta, average_reward, arm_id))
        conn.commit()
...
def select_arm():
    ...
    if np.random.normal(0,1) <= epsilon:
        ...
        cur.execute("SELECT arm_id FROM armsrewardaeg ORDER BY average_reward DESC LIMIT 1")
        row = cur.fetchone()
...
def index():
    ...
    cur.execute("SELECT lm_title FROM arms10aeg WHERE arm_id = %s", (arm_id_r,))
    rows = cur.fetchall()
    ...
    if request.method == 'POST':
        ...
        if search_query.strip():
            ...
            cur.execute("SELECT arm_id FROM armsrewardaeg WHERE lower(lm_title) LIKE lower(%s) ORDER BY average_reward DESC LIMIT 1",
                        ('%{}%'.format(search_query),))
            arm_id_s = cur.fetchone()[0] if cur.rowcount > 0 else None

            if arm_id_s:
                cur.execute("SELECT lm_title FROM armsrewardaeg WHERE arm_id = %s", (arm_id_s,))
                rows = cur.fetchall()
...
def click_lm(lm_title):
    ...
    cur.execute("SELECT description FROM arms10aeg WHERE lm_title = %s", (lm_title,))
    row = cur.fetchone()
...
def click_searchquery(lm_result):
    session_key = session.get('key')
    # Retrieve the description of the clicked learning material
    cur.execute("SELECT description FROM arms10aeg WHERE lm_title = %s", (lm_result,))
    row = cur.fetchone()
    description = row[0] if row else "Description not found"
    ...
    if update_count > 1:
        ...
        return render_template('material.html', description=description)
    # Update the armsreward table with the arm_id
    cur.execute("SELECT arm_id FROM armsrewardaeg WHERE lm_title = %s", (lm_result,))
    row = cur.fetchone()
